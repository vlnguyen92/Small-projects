...
public class plotActivity extends Activity implements OnTouchListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        float[] data = b.getFloatArray(key);
        series[0] = new SimpleXYSeries(key);
            populateSeries(series[0], data);
...
//                        Color.rgb(0, 0, 150), null));
    ...
    private void populateSeries(SimpleXYSeries series, float[] data) {
        for(int i = 0; i < SERIES_SIZE; i++) {
            series.addLast(i, data[i]);
        }
    }
